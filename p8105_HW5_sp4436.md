...
The probability of at least one shared birthday increases rapidly with
group size. Around 23 people, the probability exceeds 50%, and by 50
people it is over 95%. It is nearly zero for small groups (fewer than 10
people) and it rises quickly afterward.
...
across all simulations, the average μ̂ closely matches the true μ. The
purple line, which conditions on rejecting the null, is biased upward
for small μ because only unusually large samples yield p \< 0.05. As μ
grows and power approaches 100%, the conditional and unconditional
averages converge.
...
The dataset includes victim demographics (name, age, sex, race),
incident date, location (city and state), geographical coordinates, and
case disposition.
